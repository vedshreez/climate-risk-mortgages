...
if __name__ == "__main__":
    ...
    sonoma_orig = load_origination("data/raw/northbay_orig_2017Q1Q2.txt", disaster_exposed=1)
    sac_orig = load_origination("data/raw/centralvalley_orig_2017Q1Q2.txt", disaster_exposed=0)
    orig = pd.concat([sonoma_orig, sac_orig], ignore_index=True)
    print(f"Total loans: {len(orig)} (North Bay (Sonoma+Napa): {len(sonoma_orig)}, Central Valley (Sacramento+Stockton): {len(sac_orig)})")

    print("Building target variable from performance data (Oct 2017-Feb 2018 window)...")
    target = load_performance_target([
        "data/raw/northbay_perf_2017Q1.txt", "data/raw/northbay_perf_2017Q2.txt",
        "data/raw/centralvalley_perf_2017Q1.txt", "data/raw/centralvalley_perf_2017Q2.txt",
    ])
    ...
    with open("outputs/northbay_model_performance.txt", "w") as f:
    ...
    mean_abs_shap.to_csv("outputs/northbay_shap_feature_importance.csv")

    plt.figure()
    shap.summary_plot(shap_values, X_test, show=False)
    plt.tight_layout()
    plt.savefig("outputs/northbay_shap_summary_plot.png", dpi=150, bbox_inches="tight")
    print("\nSaved outputs/northbay_shap_summary_plot.png")
